...
void HoleListModel::drawHoles() const
{
	...
	for( ; it != holesManager.holes.end(); ++it)
	{
		...
		{
			if(it->IsAccepted())
                glColor((Color4d)Color4b::DarkGreen);
			else
                glColor((Color4d)Color4b::DarkRed);
		}
		else
            glColor((Color4d)Color4b::DarkBlue);
		it->Draw();
	}
	...
	if(!pickedAbutment.IsNull())
	{
		...
		glLineWidth(2.0f);
        glColor((Color4d)Color4b::Yellow);
		glBegin(GL_LINES);
	...
	for(it = holesManager.holes.begin(); it != holesManager.holes.end(); ++it)
	{
		...
		{
			if(it->IsAccepted())
                glColor((Color4d)Color4b::Green);
			else
                glColor((Color4d)Color4b::Red);
		}
		else
            glColor((Color4d)Color4b::Blue);
