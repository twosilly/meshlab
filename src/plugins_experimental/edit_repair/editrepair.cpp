...
MatrixXf RotationVector(Point3f &srcv,Point3f &dstv){
    ...
    MatrixXf mf(3,1);
    mf =matrix.transpose()*vtemp;//??????????
    Point3f dstv1 = Point3f(mf(0,0),mf(1,0),mf(2,0));

    float dis = Distance(dstv,dstv1);//?????dstv???????¦Ë????
...
void EditRepairPlugin::Decorate(MeshModel &m, GLArea * gla, QPainter *p)
{
    ...
    if(crownBoundaryT.size()>0 && stagesDialog->checkBoxShowPointsState() == Qt::Checked)
    {
        glPushMatrix();
        glMultMatrix(m.cm.Tr);
        glPushAttrib(GL_ENABLE_BIT | GL_CURRENT_BIT | GL_LINE_BIT | GL_DEPTH_BUFFER_BIT);
    ...
    if(crownInnerT.size()>0 && stagesDialog->checkBoxShowPointsState() == Qt::Checked)
    {
        glPushMatrix();
        glMultMatrix(m.cm.Tr);
        glPushAttrib(GL_ENABLE_BIT | GL_CURRENT_BIT | GL_LINE_BIT | GL_DEPTH_BUFFER_BIT);
    ...
    if(rootBoundaryT.size()>0 && stagesDialog->checkBoxShowPointsState()== Qt::Checked)
    {
        glPushMatrix();
        glMultMatrix(m.cm.Tr);
        glPushAttrib(GL_ENABLE_BIT | GL_CURRENT_BIT | GL_LINE_BIT | GL_DEPTH_BUFFER_BIT);
    ...
    for(vitd = mesh_tem_d->cm.vert.begin(); vitd != mesh_tem_d->cm.vert.end(); ++vitd){
        ...
        if ( c1 != Color4<float>(Color4<float>::Red))//if white color?
        {
            ...
            if(!(*vitd).IsD()){//?§Ø??????????
                ...
                CVertexO* firstV = pos.VFlip();//???vitd???????????
                CVertexO* tempV= NULL;
                do
                {
                    pos.NextE();
...
void EditRepairPlugin::slotDeletePro()
{
    ...
    CMeshO::FaceIterator fi;
    CMeshO::VertexIterator vi;
    tri::UpdateSelection<CMeshO>::VertexClear(mesh_tem_d->cm);
    ...
    mesh_tem_d->clearDataMask(MeshModel::MM_FACEFLAG);
}
...
void EditRepairPlugin::slotImportSet()
{
    ...
    if (inputs.is_open())
    {
        ...
        stagesDialog->SetToothinfoText(QString("Max: ")+QString("%1").arg(max)+QString("; Min: ") + QString("%1").arg(min));
        stagesDialog->ToothinfoAdjustSize();
...
{
    mesh->updateDataMask(MeshModel::MM_FACEFLAG);
    borders.clear();
    CMeshO::VertexIterator vi;
    int i = 0;
    ...
    mesh_tem_d->updateDataMask(MeshModel::MM_FACEFLAG);
    borders_root.clear();
    i = 0;
    for(vi = mesh_tem_d->cm.vert.begin(); vi != mesh_tem_d->cm.vert.end(); ++vi){
        if( !(*vi).IsD() )
...
void EditRepairPlugin::sortTeeth()//Sort TeethL from L to R and Sort TeethU from R to L?????????????????????????????
{
...
void EditRepairPlugin::slotDeformationLaplacian(){
    ...
    int VN;//??????????????
    int flag;
    CMeshO::PerVertexAttributeHandle<int> col = tri::Allocator<CMeshO>::AddPerVertexAttribute<int>(mesh_tem_d->cm ,std::string("v_col"));//?????????????col??????????????????????????§Ö??§Ü?
    ...
    SparseMatrix<float> A0;//Laplacian????
    A0.resize(VN,VN);
    ...
    SparseMatrix<float> A;//????Laplacian????????
    A.resize(VN1,VN);
...
void EditRepairPlugin::slotCut(){
    ...
    float perStep = stagesDialog->ui.doubleSpinBoxPerStep->value();//???????
    float planeOffset = stagesDialog->ui.doubleSpinBoxPlaneOffset->value();//???????????
    int cut_times = stagesDialog->ui.spinBoxCutTimes->value();//????§Ú?????
    ...
    {
        planeCenter = barycentre+planeAxis*(planeOffset + i*perStep);//???????????
...
void EditRepairPlugin::slotMerge(){
    ...
    std::vector<int> VertexId(mesh->cm.vert.size());//mesh_origin??????§Ö?mesh??VertexId?????????????????????????????????????mesh?????????????????§Ö????????????????
    int numvert = 0;
    std::vector<int> VertexId2(mesh2->cm.vert.size());//mesh_origin??????§Ö?mesh??VertexId?????????????????????????????????????mesh?????????????????§Ö????????????????
    int numvert2 = 0;

    for (vi=mesh->cm.vert.begin();vi!=mesh->cm.vert.end();vi++){//????mesh_origin?????
        if (!(*vi).IsD())
        {
    ...
    for(fi=mesh->cm.face.begin(); fi!=mesh->cm.face.end(); ++fi)//????mesh_origin?????
    {
        if( !(*fi).IsD() ){
            vf_mm.push_back(*fi);
        }
    ...
    for (vi=mesh2->cm.vert.begin();vi!=mesh2->cm.vert.end();vi++){//????mesh_origin?????
        if (!(*vi).IsD())
        {
    ...
    for(fi=mesh2->cm.face.begin(); fi!=mesh2->cm.face.end(); ++fi)//????mesh_origin?????
    {
        if( !(*fi).IsD() ){
            vf_mm2.push_back(*fi);
        }
    ...
    MeshModel *mesh_copy = md->addNewMesh("mesh_copy","",false);//???mesh_copy
    mesh_copy->cm.Clear();
    int VN,FN;
    ...
    for(itv=vv_mm2.begin();itv!=vv_mm2.end();itv++){//???????
        ivp[cnt]=&*vi;
        (*vi).P()= (*itv).P();
        ++vi;
        ++cnt;
    ...
    for (itf = vf_mm2.begin(); itf!= vf_mm2.end(); itf++)//???????
    {
        for(int k=0;k<(*itf).VN();k++)
        {
            int vInd = VertexId2[(*itf).V(k)-&*(mesh2->cm.vert.begin())] + vv_mm.size();//??????????????
            (*fi).V(k)= ivp[vInd];
        }
        ++fi;
    }
...
void EditRepairPlugin::slotFindNeastestPoints()
{
    ...
    mesh->updateDataMask(MeshModel::MM_FACEFLAG);
    mesh2->updateDataMask(MeshModel::MM_FACEFLAG);
...
void EditRepairPlugin::slotExtendBorder()
{
    if (1)
    {
        ...
        int num = 1;
        while (tempVLast != &mesh->cm.vert[flag2])//????????????
        {
            teethBordersInOrder.push_back(tempVLast-&*(mesh->cm.vert.begin()));
            CVertexO *vi0 = tempVLast;
            vcg::face::JumpingPos<CFaceO> pos0(vi0->VFp(),vi0);
    ...
    if (1)
    {
        ...
        int num = 1;
        while (tempVLast != &mesh2->cm.vert[flagH2])//????????????
        {
